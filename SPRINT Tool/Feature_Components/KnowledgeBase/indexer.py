...
from .parser import PythonParser, FunctionInfo, ClassInfo
...
class RepositoryIndexer:
    ...
    def __init__(self, model_name: str = "microsoft/unixcoder-base",
        ...
        logger.info("RepositoryIndexer initialized")
    ...
    def index_repository(self, repo_path: str, repo_name: str) -> IndexResult:
        ...
        for py_file in python_files:
            try:
                # Parse file
                tree = self.parser.parse_file(str(py_file))
                if tree is None:
                ...
                # Get relative path
                rel_path = py_file.relative_to(repo_path_obj)
                ...
                # Extract classes
                classes = self.parser.extract_classes(tree, source_code, str(py_file))
                for cls in classes:
                ...
                # Extract functions
                functions = self.parser.extract_functions(tree, source_code, str(py_file))
                ...
                calls_map = self.parser.extract_calls(tree, source_code)
                ...
                logger.error(f"Failed to process file {py_file}: {e}")
                failed_files.append(str(py_file))
